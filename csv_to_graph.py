...
CSV_PATH = Path("/home/nokdujeon/kangseok/ILLIXR/build/nsys_log/periodic_log.csv")
...
def parse_time_column(df: pd.DataFrame, time_col: str):
    ...
    # 1) 문자열/숫자 혼합 datetime 파싱 시도
    parsed = pd.to_datetime(df[time_col], errors="coerce", infer_datetime_format=True)
    if parsed.notna().sum() >= max(3, int(0.5 * len(df))):
        df["_time_parsed"] = parsed
        return "_time_parsed", True

    # 2) 숫자형이면 epoch(s/ms) 가정
